    ...
    if (currentIcon != NONE) {
        if (max_stage != -1) {
            ...
            for (int i = 0; i < max_stage; ++i) {
                ...
                gr_blit(stage_surface, 0, 0, stage_width, stage_height, x, 0);
                x += stage_width;
            }
        }
        ...
        int text_y = gr_fb_height() - gr_get_height(text_surface);
        gr_color(255, 255, 255, 255);
        gr_texticon(text_x, 4*text_y/5, text_surface);
    }
}
...
void ScreenRecoveryUI::draw_foreground_locked() {
    ...
    if (progressBarType != EMPTY) {
        ...
        int progress_y = 5*(gr_fb_height()-height)/6;
