...
static int mt_update_active_part(State* state, char* from_partition, char *to_partition)
{
    if (!strncmp(from_partition, "tee", strlen("tee")) ) {         // tee1 and tee2 partition not use active bit
        if (!strcmp(from_partition, "tee1") ) // only do erase when main partition to alt partition
            return mt_update_erase_part(state, from_partition);
    }else {
        // TODO: other partitions from preloader/tee,   need to set to_partition active bit to 1 and then set from_partition active bit to 0.    (gpt type &  pmt type both need.)
    }
    return 1;
}
